...
#pragma omp parallel num_threads(n_threads)
    {
        ...
        int items_per_thread = m / n_threads; //  6 / 4 = 1 (bcs int, else 1.5)
        int start_pos = thread_id * items_per_thread; //  0 * 1 = 0, 1 * 1 = 1, 2 * 1 = 2, 3 * 1 = 3
        int end_pos = (thread_id == n_threads - 1) ? (m - 1) : (start_pos + items_per_thread - 1);
        //  Calculations
...
void run_parallel(size_t m, size_t n) {
...
#pragma omp parallel num_threads(n_threads)
    {
        ...
        int end_pos = (id_thread == n_threads - 1) ? (m - 1) : (start_pos + items_per_thread - 1);
        ...
        end_pos = (id_thread == n_threads - 1) ? (n - 1) : (start_pos + items_per_thread - 1);
